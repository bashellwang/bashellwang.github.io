...
考虑到我们项目多渠道包主要针对不同国家版本，每个国家一个渠道包，虽然整体代码是同一套，但是所有渠道包代码不能保证一致，因此无法通过一个补丁完成所有渠道修复工作。这里我们项目采用的是 productFlavors 这种方式。
...
tinkerid 是 tinker 识别每一个 apk 包的唯一标识，它被写入到 `AndroidManifest` 文件中。
